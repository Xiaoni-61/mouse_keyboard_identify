...
def load_data(t):
    ...
    for i in range(1):
        for j in range(2):
            ...
            mouse_path = 'data/mouse_data/' + peopleName[j] + '_' + (str)(t) + '0s/' + peopleName[j] + '_' + \
                   mouseEventName[i] + '_' + (str)(t) + '0s' + ".txt"

            keyboard_path = 'data/keyboard_data/' + keyboardEventName[i] + '_' + peopleName[j] + '_' + (str)(t) + "0s/" + \
                   keyboardEventName[i] + '_' + peopleName[j] + '_' + (str)(t) + "0s" + ".txt"

            logging.info("Loading: " + peopleName[j] + mouseEventName[i])
            data1 = file1array(mouse_path)

            logging.info("Loading: " + peopleName[j] + keyboardEventName[i])
            data2 = file2array(keyboard_path)

            # # 横向拼接
            # np.hstack((n1, n2))
            # # 或者（axis = 1 表示行）
            # np.concatenate((n1, n2), axis=1)
            data = np.hstack((data1[:59],data2))
            if j == 0 and i == 0:
                sum = data
            else:
...
def file2array(path, delimiter=' '):  # keyboard
    fp = open(path, 'r', encoding='GBK')
    string = fp.read()  # string是一行字符串，该字符串包含文件所有内容
    fp.close()
    row_list = string.splitlines()  # splitlines默认参数是‘\n’
...
if __name__ == "__main__":
    ...
    for t in range(1, 6):
        data = load_data(t)
        x, y, z = np.split(data, indices_or_sections=(48553, 48555), axis=1)  # x为数据，y为标签,axis是分割的方向，1表示横向，0表示纵向，默认为0
